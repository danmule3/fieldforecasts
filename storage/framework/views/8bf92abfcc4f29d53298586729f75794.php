<section class="py-8 bg-slate-100/60 dark:bg-slate-900/60">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h2 class="text-xl font-bold"><?php echo e($section->title ?? 'Recent winners'); ?></h2>
            <div class="text-sm">
                Overall prediction accuracy:
                <span class="font-bold text-lg text-indigo-600 dark:text-indigo-400"><?php echo e($accuracy); ?>%</span>
            </div>
        </div>

        <?php if($recentWinners->isEmpty()): ?>
            <p class="text-sm text-slate-500 dark:text-slate-400">No settled predictions yet.</p>
        <?php else: ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <?php $__currentLoopData = $recentWinners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prediction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php /**PATH C:\Sites\fieldforecasts\resources\views/partials/sections/recent_winners.blade.php ENDPATH**/ ?>